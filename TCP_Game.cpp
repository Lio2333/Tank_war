...
void CGame::gamethread()
{
	...
	while (1)
	{
		...
		if (fd_num > 0)//处理服务器发送回来的信息
		{
			...
			else if (state == 3)//play2 子弹位置的获取调整   //子弹获取位置不需要！
			{
				//可能需要修改优化
				CBullet bult;
...
void CGame::GameRunLogicOnOne2OneMenu()
{
  ...
  //  if (KEYDOWN('Q')) {
    ...
    if (m_keymenu01.GetStop() && m_keymenu02.GetStop()) {
		...
		HANDLE hThread = CreateThread(NULL, 0, RecvSer, this, 0, NULL);//创建线程 用于传输接收信息
