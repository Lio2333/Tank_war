...
public:
    CGame();
    ~CGame();
	PointF pp;//记录坦克位置并输出用于调试
	PointF inittank;//用于线程中初始化坦克位置
	int initnum = 0;
	HANDLE hThread;
	int threadint = 0;

	//std::string decode_tank02="0";//记录02坦克位置用于传输

	 // DWORD WINAPI RecvSer(LPVOID lpParamter);//多线程 用于接收并处理服务器信息。
	void gamethread();
...
private:
    ...
    */
    ...
    bool (CGame::*m_initFunc[EGameTypeCount])() = {
        ...
        &CGame::GameInitOne2BotMenu, //单人对电脑，菜单阶段
        &CGame::GameInitOne2Bot, // 单人对电脑
        &CGame::GameInitOne2BotEnd, // 单人对电脑 结束
        &CGame::GameInitOne2OneMenu, //双人对战，菜单阶段
    ...
    void(CGame::*m_logicFunc[EGameTypeCount])() = {
        ...
        &CGame::GameRunLogicOnOne2BotMenu, //单人对电脑，菜单阶段
        &CGame::GameRunLogicOnOne2Bot, //单人对电脑
        &CGame::GameRunLogicOnOne2BotEnd, //单人对电脑 结束
        &CGame::GameRunLogicOnOne2OneMenu, // 双人对战，菜单阶段
